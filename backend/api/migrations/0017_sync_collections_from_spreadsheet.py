...
DATA_FILE = Path(__file__).resolve().parent / "data" / "collections_spreadsheet_seed.json"
...
def sync_spreadsheet_collections(apps, schema_editor):
...
def reset_spreadsheet_collections(apps, schema_editor):
    Collection = apps.get_model("api", "Collection")
    CollectionSection = apps.get_model("api", "CollectionSection")

    payload = load_seed_payload()
    Collection.objects.filter(
        slug__in=[collection["slug"] for collection in payload["collections"]]
    ).delete()
    CollectionSection.objects.filter(
        slug__in=[section["slug"] for section in payload["sections"]]
    ).delete()
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("api", "0016_collectionsection_alter_collection_section_and_more"),
    ]

    operations = [
        migrations.RunPython(sync_spreadsheet_collections, reset_spreadsheet_collections),
    ]
