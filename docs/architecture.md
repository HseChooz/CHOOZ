...
- `Note` — быстрые заметки пользователя с признаком избранного, ссылкой и временными метками создания и обновления;
...
Домен `notes` уже оформлен как отдельный backend-срез:

- есть queries `notes(onlyFavorites: ...)` и `note(id: ...)`;
- есть мутации `createNote`, `updateNote`, `deleteNote`;
- заметки изолированы по владельцу и недоступны другим пользователям;
- сервисный слой умеет автоматически извлекать первую ссылку из текста заметки, если она не передана отдельным полем;
- заметки сортируются по `updated_at`, а для избранного и сортировки уже заведены отдельные индексы.

По состоянию кода это не заготовка, а работающий backend-домен с моделью, миграцией и отдельным тестовым покрытием.
...
Покрываются auth, wishlist, notes, events и collections.
...
- Заметки и подборки уже оформлены как самостоятельные backend-срезы, хотя на iOS они ещё не доведены до полноценного пользовательского интерфейса.
...
- Backend ушёл дальше MVP и уже содержит отдельные домены заметок и подборок, причём подборки дополнительно поддерживают поиск, теги и собственные ассеты.
- Основное асинхронное расхождение между слоями сейчас в том, что backend уже оформил несколько v2-доменов, а основной iOS UI ещё сосредоточен на wishlist/calendar/profile flow.
